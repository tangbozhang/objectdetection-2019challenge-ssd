...
def coco(settings, coco_api, file_list, mode, batch_size, shuffle, data_dir):
    ...
    catid2clsid = dict(
        {catid: i + int(1)
         for i, catid in enumerate(coco_api.getCatIds())})
    cname2cid = dict({
    ...
    def reader():
        ...
        for image in file_list:
            ...
            for ann in anns:
                ...
                bbox_sample.append(float(catid2clsid[ann['category_id']]))
                bbox = ann['bbox']
                xmin, ymin, w, h = bbox
                xmax = xmin + w
                ymax = ymin + h
...
def train(settings,
    ...
    if use_multiprocess:
        return paddle.reader.multiprocess_reader(readers, False, 10)
    else:
        return readers[0]
...
def test(settings, file_list, batch_size):
    ...
    if 'coco' in settings.dataset:
        ...
        image_ids = coco_api.getImgIds()
        print("coco image_ids",len(image_ids))
        images = coco_api.loadImgs(image_ids)
        if '2014' in file_list:
            sub_dir = "val2014"
        elif '2017' in file_list:
